#+TITLE: Org-SuperTag 的一天 — 一个人的完整工作流
#+AUTHOR: Org-SuperTag 社区
#+EMAIL: yibie@users.noreply.github.com
#+OPTIONS: toc:3 num:t

* 如何使用本文档

这不是功能列表。这是 /一个人真实使用 Org-SuperTag 的日常/，仿照 Bernt Hansen 的经典 [[http://doc.norang.ca/org-mode.html][org-mode 教程]] 风格写成。
...
本文档假设你已经安装了 Org-SuperTag。安装说明见 [[file:../README_CN.md][README]]。

如果你想看英文版：[[file:A-DAY-WITH-ORG-SUPERTAG.org][A Day with Org-SuperTag (English)]]

* 我的环境配置

** 我的 Org 文件

我把生活放在 =~/org/= 下的几个文件中。每个文件有清晰的目的，而 Org-SuperTag 把它们当作一个统一的知识库。
...
为什么分开多个文件？因为 Org-SuperTag 可以 /跨所有文件/ 查询。文件分隔是为了 /我/ 的脑袋——Org-SuperTag 不在乎标题在哪个文件里。=meetings.org= 里的 =#task= 和 =projects.org= 里的 =#task= 在表格视图中并列显示。 
** Vault 配置
...
这是 Org-SuperTag 相对纯 Org-mode 最大的优势。捕获 /快/ 且 /结构化/——不需要记忆字段名或语法。
...
自动化的意义：我定义逻辑 /一次/，Org-SuperTag 每次自动应用。"哎呀，忘了给会议记录填日期"这种事不会再发生。

* 工作中：像 App 一样的视图

** Table View——我的主操作界面

我 60% 的 Org-SuperTag 时间都在 Table View 里。它用起来像一个轻量级数据库客户端，但完全在 Emacs 里。
...
我拖拽任务在各列之间推进。Org-SuperTag 自动更新 =status= 字段。如果我设置了 =status= 变更的自动化规则（比如记录完成时间），它们会立刻触发。
...
这在当前节点和目标节点之间创建双向链接。引用会出现在两个节点的 Table View 的 =Refs= 列中。

#+begin_src emacs-lisp
  ;; 引用在数据库中存储为 :reference 关系。
  ;; 它独立于 Org 的 [[链接]]——这意味着：
  ;;   1. 文件重组后仍然存在
  ;;   2. 可查询（找到所有引用节点 X 的节点）
...
备份存储在 =~/.emacs.d/org-supertag/backups/=。
...
#+begin_src emacs-lisp :tangle supertag-workflow.el
  ;;; supertag-workflow.el — 我的 Org-SuperTag 日常配置
...
- =org-supertag-sync-directories-mode 'unified= 配 10+ 目录 → 切换到了 vault 模式。
  每次 tick 扫描所有目录太慢了。
...
- [[file:A-DAY-WITH-ORG-SUPERTAG.org][英文版教程]]
...
- [[file:ONTOLOGY-ARCHITECTURE_cn.md][架构深度解析]]
- [[file:ORG-SUPERTAG-PLUGIN-GUIDE_cn.md][插件开发指南]] — 构建自己的 extractor 和服务
