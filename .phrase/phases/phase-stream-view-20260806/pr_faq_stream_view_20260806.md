...
Stream View 把标签当作浏览入口。一个单列主 buffer 在同一行显示节点创建日期、对应 `#tag` 与可识别标题，不显示文件路径、Org 星号、正文或带下划线的 button。当前行仅使用轻微背景高亮表示选择。
...
每行显示节点创建日期、全部对应 `#tag`、可读标题和轻微选中态，不显示源文件路径、Org 星号、正文或 button 下划线。
...
- 单列日期/标签/标题、导航和稳定位置恢复可用；不存在 companion index、`s` 或 button 下划线。
...
4. 标题流显示日期、全部对应标签、标题和轻微选中态；不显示路径、正文、Org 星号或 button 下划线。
