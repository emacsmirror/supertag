...
2. main buffer 每行显示 `[YYYY-MM-DD Day HH:MM]  #tag…  标题`；不显示正文、文件路径、Org 星号或带下划线的 button。
...
task013 追加检查：在窗口中部标题按 `n`/`p` 时行位置自然移动；折叠源节点后按 `e` 仍看到标题与正文；分别用 `C-c C-k` 验证取消、`C-c C-c` 验证确认。task014 追加检查：每一行的日期、对应标签和标题可读，且不重新出现正文/index/button。issue037 在用户明确通过前保持打开。
